...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
static rf_pulse_t pulses[RF_STM32_MAX_PULSES];
...
void rf_test(void)
{
    ...
    status = rf_driver_receive_start(rf);
    ...
    /*
     * Temporary test.
     * Read after a short capture period.
     */
    HAL_Delay(200);

    rf_driver_receive_stop(rf);

    uint32_t count = 0U;

    status = rf_driver_receive_read(
        rf,
        pulses,
        RF_STM32_MAX_PULSES,
        &count);
